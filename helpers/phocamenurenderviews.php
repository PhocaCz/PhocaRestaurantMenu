...
class PhocaMenuRenderViews
{
	...
	public static function renderDailyMenu($data, $tmpl, $params, $paramsG, $method = 0) {
		...
		if ($method == 3) {
			$o = '<div id="phocamenumultipleedit">';
		} else {
			$o = '<div id="phocamenu">';
		}
	...
	public static function renderFormItem($type, $tag, $image, $itemObject, $price, $pricePref, $method, $price2, $pricePref2, $displaySecondPrice, $suffix = '') {
		...
		if ($method == 4) {
			...
			$row['title'] = '<div class="span'.$titleColumn.' pmtitle '.$pmtitleO.'">'.$itemObject->title.'</div>';
			
			$o = $tag['row-o'] . $row['img'] . $row['quantity'] . $row['title'] . $row['prefix'] . $row['price1'] . $row['price2'] . $tag['row-c'];
			...
			if ($itemObject->description != '') {
				...
				$descRow .= '<div class="span'.$titleColumn.' pmdesc">';
