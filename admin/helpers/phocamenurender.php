...
class PhocaMenuRender
{
	...
	public static function quickIconButton( $component, $link, $image, $text ) {
		...
		public static function renderScreenshotScript($id, $params) {
			...
			$oLang = array();
			$oLang['COM_PHOCAMENU_SUCCESS_IMAGE_COPIED'] = JText::_('COM_PHOCAMENU_SUCCESS_IMAGE_COPIED');
            $oLang['COM_PHOCAMENU_ERROR_IMAGE_COPIED'] = JText::_('COM_PHOCAMENU_ERROR_IMAGE_COPIED');
			JFactory::getDocument()->addScriptOptions('phLangPRM', $oLang);
			JFactory::getDocument()->addScriptOptions('phVarsPRM', $oVars);

			Joomla\CMS\HTML\HTMLHelper::_('script', 'media/com_phocamenu/js/html2canvas.min.js', array('version' => 'auto'));
			Joomla\CMS\HTML\HTMLHelper::_('script', 'media/com_phocamenu/js/screenshot.js', array('version' => 'auto'));
	...
	public static function renderScreenshot() {
	    ...
	    $pc             = JComponentHelper::getParams('com_phocamenu');
	    ...
	    if ($app->isClient('administrator')) {
            ...
            return '<button type="button" class="phPrintButton">' . JText::_('COM_PHOCAMENU_TAKE_SCREENSHOT') . '</button><div style="clear:both"></div>';
        }
