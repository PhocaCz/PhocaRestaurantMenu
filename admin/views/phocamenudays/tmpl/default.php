...
$linkView		= Route::_( 'index.php?option='.$this->t['o'].'&view='.$this->t['c'].'groups&type='.(int)$this->type['value'].'&did='.$item->id );
...
$date = PhocaMenuHelper::getDate($item->title, $this->t['daydateformat'], $this->t['dateclass']);

if ($canCreate || $canEdit) {
	$checkO .= '<a href="'. Route::_($linkEdit).'">'. $this->escape($date).'</a>';
} else {
	$checkO .= $this->escape($date);
}
...
if ($canCreate || $canEdit) {
	$o[] = '<span class="ph-editinplace-text ph-eip-date ph-eip-title" id="day' . ':' .'title'.':'.(int)$item->id . '">' . PhocaText::filterValue(str_replace("00:00:00", '', $item->title), 'text') . '</span>';


	$o[] = '<span class="" id="day' . ':' .'dateformat'.':'.(int)$item->id . '">' . $date . '</span>';
} else {
	$o[] = $this->escape($date);
}
...
$vO = '<a class="ph-inline-task" href="'. $linkView.'" title="'. Text::_('COM_PHOCAMENU_VIEW_DAY_GROUPS').'">'
	//. JHtml::_('image', $this->t['i'].'icon-16-item.png', JText::_('COM_PHOCAMENU_VIEW_DAY_GROUPS') )
...
$vD = '<a href="'. $linkRemove.'" onclick="'.$onClickRemove.'" title="'. Text::_('COM_PHOCAMENU_DELETE').'"'
	.' onclick="return confirm(\''.Text::_('COM_PHOCAMENU_WARNING_DELETE_DAY').'\');">'
...
if ($this->t['displayitemtools'] == 1) {
	...
	//.JHtml::_('image', $this->t['i'].'icon-16-preview.png', JText::_('COM_PHOCAMENU_PREVIEW'), 'style="max-width: none"' ).'</a>';
