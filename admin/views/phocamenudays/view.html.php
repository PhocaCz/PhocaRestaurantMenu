...
class PhocaMenuCpViewPhocaMenuDays extends HtmlView
{
	protected $items;
	...
	function display($tpl = null) {
		...
		$this->t			= PhocaMenuUtils::setVars('day');
		...
		$this->type			= PhocaMenuHelper::getUrlType('day');
		...
		$this->t['dateclass']		= $params->get( 'date_class', 0 );
		$this->t['daydateformat']	= $params->get( 'day_date_format', 'l, d. F Y' );
		$this->t['weekdateformat']	= $params->get( 'week_date_format', 'l, d. F Y' );
		$this->t['displayitemtools']= $params->get( 'display_item_tools', 1 );
		...
		$this->t['linkemail'] 	= 'index.php?option=com_phocamenu&task=phocamenuemail.edit&type='.$this->type['value'].'&typeback=day';
		$this->t['linkmultiple']= 'index.php?option=com_phocamenu&task=phocamenumultipleedit.edit&type='.$this->type['value'].'&typeback=day';
		$this->t['linkraw']= 'index.php?option=com_phocamenu&task=phocamenurawedit.edit&type='.$this->type['value'].'&typeback=day';
		$this->t['linkpdf']		= PhocaMenuRender::getIconPDFAdministrator($this->type['info']['frontview'], 1, $langSuffix);
		$this->t['linkconfig']	= 'index.php?option=com_phocamenu&task=phocamenuconfig.edit&type='.$this->type['value'].'&typeback=day';
		$this->t['linkedit']	= 'index.php?option=com_phocamenu&task=phocamenuday.edit&type='.(int)$this->type['value'];
		$this->addToolbar();
		parent::display($tpl);
	}
	...
	protected function addToolbar() {
		...
		$this->type['actualcatid']	= PhocaMenuHelper::getActualCategory('day', $this->type['value'], $categoryId);

		require_once JPATH_COMPONENT.'/helpers/phocamenudays.php';
		$canDo	= phocamenudaysHelper::getActions($this->t);
		...
		if ($canDo->get('core.create')) {
			ToolbarHelper::addNew( 'phocamenuday.add','JToolbar_NEW');
		}
		if ($canDo->get('core.edit')) {
			ToolbarHelper::editList('phocamenuday.edit','JToolbar_EDIT');
			if ($displayToolbars) {
		...
		if ($canDo->get('core.edit.state')) {
			ToolbarHelper::divider();
			ToolbarHelper::custom('phocamenudays.publish', 'publish.png', 'publish_f2.png','JToolbar_PUBLISH', true);
			ToolbarHelper::custom('phocamenudays.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JToolbar_UNPUBLISH', true);
		}

		if ($canDo->get('core.delete')) {
			ToolbarHelper::deleteList( 'COM_PHOCAMENU_WARNING_DELETE_ITEMS', 'phocamenudays.delete', 'COM_PHOCAMENU_DELETE');
		}
