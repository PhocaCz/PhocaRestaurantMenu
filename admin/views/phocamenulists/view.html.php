...
class PhocaMenuCpViewPhocaMenuLists extends HtmlView
{
	protected $items;
	...
	function display($tpl = null) {
		...
		$this->t['linkemail'] 	= 'index.php?option=com_phocamenu&task=phocamenuemail.edit&type='.$this->type['value'].'&typeback=list';
		$this->t['linkmultiple']= 'index.php?option=com_phocamenu&task=phocamenumultipleedit.edit&type='.$this->type['value'].'&typeback=list';
		$this->t['linkraw']= 'index.php?option=com_phocamenu&task=phocamenurawedit.edit&type='.$this->type['value'].'&typeback=list';
		$this->t['linkpdf']		= PhocaMenuRender::getIconPDFAdministrator($this->type['info']['frontview'], 1, $langSuffix);
		$this->t['linkconfig']	= 'index.php?option=com_phocamenu&task=phocamenuconfig.edit&type='.$this->type['value'].'&typeback=list';
		$this->t['linkedit']	= 'index.php?option=com_phocamenu&task=phocamenulist.edit&type='.(int)$this->type['value'];
		$this->addToolbar();
		parent::display($tpl);
	}
	...
	protected function addToolbar() {
		...
		$this->type['actualcatid']	= PhocaMenuHelper::getActualCategory('list', $this->type['value'], $categoryId);

		require_once JPATH_COMPONENT.'/helpers/phocamenulists.php';
		$canDo	= PhocaMenuListsHelper::getActions($this->t);
		...
		if ($canDo->get('core.create')) {
			ToolbarHelper::addNew( 'phocamenulist.add','JToolbar_NEW');
		}
		if ($canDo->get('core.edit')) {
			ToolbarHelper::editList('phocamenulist.edit','JToolbar_EDIT');
			if ($displayToolbars) {
		...
		if ($canDo->get('core.edit.state')) {
			ToolbarHelper::divider();
			ToolbarHelper::custom('phocamenulists.publish', 'publish.png', 'publish_f2.png','JToolbar_PUBLISH', true);
			ToolbarHelper::custom('phocamenulists.unpublish', 'unpublish.png', 'unpublish_f2.png', 'JToolbar_UNPUBLISH', true);
		}

		if ($canDo->get('core.delete')) {
			ToolbarHelper::deleteList( 'COM_PHOCAMENU_WARNING_DELETE_ITEMS', 'phocamenulists.delete', 'COM_PHOCAMENU_DELETE');
		}
