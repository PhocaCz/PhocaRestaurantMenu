...
class PhocamenuLoader extends JLoader
{
	...
	public static function import($filePath, $base = null, $key = 'libraries.') {
		...
		if (!isset($paths[$keyPath])) {
			if ( !$base ) {
				$base =  JPATH_ADMINISTRATOR.'/components/com_phocamenu/libraries';
			}
			...
			if (strpos($filePath, 'phocamenu') === 0) {
				$className	= 'PhocaMenu'.$className;
			...
			PhocamenuLoader::$paths[$keyPath] = $rs;
		}

		return PhocamenuLoader::$paths[$keyPath];
	}
}

function phocacartimport($path) {
	return PhocamenuLoader::import($path);
}
